 ...
 */
package com.tilab.ca.sse.core.util;
...
public class SSEVariables {

	static Log LOG = LogFactory.getLog(SSEVariables.class);
	...
	public SSEVariables() {
		unchecked(() -> {
			InputStream configStream = Thread.currentThread()
				...
				.getResourceAsStream("ssecore.properties");
			init(configStream);
		});
	}
	 ...
	 */
	@Deprecated
	public SSEVariables(String confFile) {
		unchecked(() -> {
	...
	private void init(InputStream confStream) {
		unchecked(() -> {
			...
			STOPWORDS_IT = SSEUtils.getStopWords(config.getProperty("stopWords.it", "").trim());
			...
			STOPWORDS_EN = SSEUtils.getStopWords(config.getProperty("stopWords.en", "").trim());
